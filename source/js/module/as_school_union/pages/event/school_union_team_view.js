...
const	Score			= require('./../../../ui/score/score');
...
const SchoolUnionTeamView = React.createClass({
	...
	renderPlayersForLeftSide: function() {
		...
		if(TeamHelper.isNonTeamSport(event)) {
			...
			if(
				...
				const teamIndex = teamsData.findIndex(t => t.schoolId === event.schoolsData[0].id);
				if(teamIndex !== -1) {
	...
	renderPlayersForRightSide: function() {
		...
		if(TeamHelper.isNonTeamSport(event)) {
			...
			if(
				...
				const teamIndex = teamsData.findIndex(t => t.schoolId === event.schoolsData[1].id);
				if(teamIndex !== -1) {
					return self.renderTeamPlayersByOrder(teamIndex, individualScoreAvailable);
				} else {
					if(
...
module.exports = SchoolUnionTeamView;